...
public class ObjectiveManager {
    ...
    private void initTeamRest(Objective objective) {
        ...
        for ( String id : members.keySet() ) {
            ...
            objective.getScore(tns.getScoreItem()).setScore(members.get(id).size());
        }
    }
    ...
    private void initZeroEachTeam(Objective objective) {
        ...
        for ( TeamNameSetting tns : parent.getAllTeamNames() ) {
            // NOTE: いったん1を設定して項目を表示させる。
            objective.getScore(tns.getScoreItem()).setScore(1);
            objective.getScore(tns.getScoreItem()).setScore(0);
        }
    }
