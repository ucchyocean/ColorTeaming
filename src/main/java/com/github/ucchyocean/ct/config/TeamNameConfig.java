...
public class TeamNameConfig {
    ...
    private TeamNameSetting ignoreTeam;
    ...
    private ArrayList<TeamNameSetting> load(File file) {
        ...
        for ( String c : contents ) {
            ...
            TeamNameSetting tns = new TeamNameSetting(id, name, color);
            config.add(tns);
            ignoreTeam = tns;
        }

        // TODO: 最低チーム設定数（10チーム）に満たない場合は、デフォルト設定で補完する。
    ...
    /**
     * 例外チーム（teamcolors.txtで一番最後に指定されたチーム）を取得する。
     * @return 例外チーム
     */
    public TeamNameSetting getIgnoreTeam() {
        return ignoreTeam;
    }
